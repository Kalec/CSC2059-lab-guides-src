...
On your own Windows computer, the CSC2059 Terminal should open automatically if you followed the \textbf{Lab 0: CSC2059 Development Environment (Pre-lab setup)}.
\end{importantnote}
...
\texttt{make test} & Builds and runs the public tests with the same function implementations.\\
\end{tabularx}

The Makefile selects the compiler, flags and output paths. You do not need to write its recipes yet. The Windows setup supplies UCRT64 GCC; on macOS, \texttt{g++} may report Apple Clang. If the supplied build requests a missing compiler, ask a demonstrator before changing the Makefile.
\end{infobox}
...
This assertion checks the result. A failed assertion may stop the test program immediately, so later checks may not have run. Read the relevant tests before implementing each function; a later failure does not necessarily mean an earlier function is still wrong.
\end{infobox}
...
Recall \texttt{.gitignore} from Practical 1. Builds can create executables and other generated files which do not belong in a source-code commit. Look for the supplied repository's \texttt{.gitignore} and read its rules alongside the Makefile and README.
...
Check that the relevant behaviour now passes and earlier working behaviour still works. Other unfinished functions may still cause failures. Since assertions may stop at the first failure, inspect which check failed before deciding what to change.
