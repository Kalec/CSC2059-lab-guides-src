...
\node[stage=CSCBlue] (read) at (90:28mm) {Read task\\and tests};
...
tests/test\_functions.cpp & Automated public tests run using \texttt{make test}.\\
.gitignore & Rules excluding generated files from Git.\\
bin/ & Generated executables; created by Make and ignored by Git.\\
\end{tabularx}
\end{tcolorbox}
...
\node[sourceblue] (header) at (-48mm,0) {\texttt{myFunctions.h}\\\footnotesize function declarations};
\node[sourcegreen] (library) at (0,0) {\texttt{myFunctions.cpp}\\\footnotesize function implementations};
\node[sourcepurple] (tests) at (48mm,0) {\texttt{tests/test\_functions.cpp}\\\footnotesize automated checks};
...
In the previous practical, you practised local Git in Visual Studio Code and prepared your GitHub account. Here you will clone your individual repository and push commits to GitHub for the first time. In this practical, you will begin writing and testing C++ code using the same development workflow that will be used throughout the module.
...
\begin{itemize}
  \item read the task and relevant tests, then run the tests before changing code;
  \item make a change;
  \item run the tests;
  \item fix any issues;
...
Read and run the relevant tests before your first change. Then follow this cycle:

\begin{center}
\textbf{Read task and tests} $\rightarrow$ \textbf{Make a small change} $\rightarrow$ \textbf{Run the tests} $\rightarrow$ \textbf{Fix problems} $\rightarrow$ \textbf{Commit} $\rightarrow$ \textbf{Push} $\rightarrow$ \textbf{Repeat}
\end{center}

The commands below preview the workflow; cloning and pushing are explained in the next sections. From the repository root, \texttt{git add .} stages additions, modifications and deletions beneath the current folder. Ignored untracked files are excluded. Check \texttt{git status} first, then inspect staged changes before committing. Section~\ref{sec:generated-files} revisits the ignore rules introduced in Practical 1.
...
On your own Windows computer, use the terminal profile configured in \textbf{Lab 0}.
\end{importantnote}

On macOS and Linux, complete the corresponding Lab 0 setup and use the normal VS Code integrated terminal.
...
Once the repository has been created, open it and locate the \textbf{Code} button. Choose HTTPS and copy the repository URL.
...
Use your file browser to create or choose a folder for your practical work (on I: in the laboratory). Open that folder using \textbf{File $\rightarrow$ Open Folder} in VS Code, then open a terminal. Run \texttt{pwd} to check your current location.

If you want a new subfolder, \texttt{mkdir CSC2059} creates it and \texttt{cd CSC2059} changes into it. Do not run \texttt{mkdir} again if it already exists. Quote paths containing spaces.

Clone the repository, replacing the quoted placeholder below with the HTTPS URL copied from GitHub:
\begin{bashcode}
git clone "YOUR_REPOSITORY_URL"
\end{bashcode}
Move into the new folder, replacing this placeholder with the actual folder name reported by the clone command:
\begin{bashcode}
...
The dot means the current folder. VS Code should open it; alternatively use File $\rightarrow$ Open Folder.
...
\begin{importantnote}
If VS Code requests permission to use its GitHub extension, follow the sign-in prompts and use the GitHub account you registered in Practical 1. Complete any browser sign-in, then return to VS Code. This can happen when cloning or pushing from either the terminal or the GUI.
\end{importantnote}
Cloning makes a local copy of the remote GitHub repository and records that remote under the name \texttt{origin}. Your edits and commits remain local until pushed. Run the following commands from the \textbf{repository root}: the top-level folder containing \texttt{Makefile} and \texttt{README.md}, not its parent or the tests subfolder.
...
\texttt{make build} & Builds both programs without running them; this is also the default for bare \texttt{make}.\\
\texttt{make run} & Builds and runs \texttt{main.cpp} with \texttt{myFunctions.cpp} for manual experiments.\\
\texttt{make test} & Builds and runs the public tests with the same function implementations, stopping at the first failed assertion.\\
\texttt{make clean} & Removes the generated \texttt{bin/} folder; source files remain.\\
\texttt{make help} & Lists the available commands.\\
\end{tabularx}

The Makefile uses C++17 and warning flags, and places compiled programs in \texttt{bin/}: \texttt{main} and \texttt{test\_functions}, with \texttt{.exe} added on Windows. The folder is created automatically. \texttt{make run-main} and \texttt{make test-functions} name the individual program and test group; \texttt{make run} and \texttt{make test} run all required groups (one each here). Saved source or header edits trigger a rebuild when needed. You do not need to write recipes or run \texttt{make clean} after ordinary edits. The Windows setup supplies UCRT64 GCC; on macOS, \texttt{g++} may report Apple Clang. If the supplied build requests a missing compiler, ask a demonstrator before changing the Makefile.
\end{infobox}
...
\begin{csccalloutbase}[unbreakable]{black!65}{CSCGray}{General Information}{\cscIconInfo}
...
\begin{infobox}
\textbf{\texttt{tests/test\_functions.cpp}}
...
This assertion checks the result. A failed assertion stops the test program immediately, so later checks have not run. The file contains checks for addition, division, logarithms, square roots and the calculator. Read them to see the full set of required behaviours. Read the relevant tests before implementing each function; a later failure does not necessarily mean an earlier function is still wrong. Practical 3 introduces Catch2 and a switch for running all test cases; this practical keeps the simpler assertion workflow. The supplied test harness also rejects a function that ends the program instead of returning to its caller.
\end{infobox}
...
\begin{importantnote}
For this practical, leave these configuration files unchanged. This is different from adding files to Git's ignore rules.
...
\subsection{Generated files and ignore rules}\label{sec:generated-files}
Recall \texttt{.gitignore} from Practical 1. Compiled programs are placed in the repository's \texttt{bin/} folder, which the supplied ignore rules exclude from Git. This is a folder inside this practical, not the operating system's \texttt{/bin}. Builds create generated files which do not belong in a source-code commit. Look for the supplied repository's \texttt{.gitignore} and read its rules alongside the Makefile and README.

After building, run \texttt{git status}. Before using \texttt{git add .}, confirm that the listed changes are your intended source changes. If generated files are listed, or the ignore file is missing, ask a demonstrator to check the supplied rules. Until then, stage only the source files you intentionally changed, for example \texttt{git add myFunctions.cpp}. Do not invent ignore patterns that might hide source files. Always inspect \mintinline{text}{git diff --staged} before committing.
...
This assertion failure is expected once the test executable has built and started: some functions are placeholders. A compiler error, missing header, missing compiler or failure to launch the executable is a different problem; resolve it before interpreting test results.
...
Check that the relevant behaviour now passes and earlier working behaviour still works. Other unfinished functions may still cause failures. Since assertions stop at the first failure, inspect which check failed before deciding what to change.
...
Once the relevant behaviour passes without breaking earlier work, commit your progress; the entire suite need not pass while other functions are unfinished.
...
The message Everything up-to-date only means there are no new commits to send; it does not upload unsaved or uncommitted edits. Save, inspect, stage and commit your work before pushing.

On your first push, open your own repository in GitHub and check the latest commit message and author. The author should link to the GitHub account you registered, using the same verified university email configured in Practical 1. If it does not, ask a demonstrator. Repeat the online commit check before leaving.
...
\begin{commonerror}
\textbf{Linker error: missing function implementation}
GCC may report \texttt{undefined reference}; Apple's linker may report \texttt{Undefined symbols for architecture arm64} (or another architecture).

For example, the header declares:
\begin{cppcode}
int iAdd(int a, int b);
\end{cppcode}
but this definition provides a different, one-argument function:
\begin{cppcode}
int iAdd(int a) { return a; }
\end{cppcode}
A call to the two-argument version still has no implementation. Check the name, parameter types and number of parameters, save the file, and ensure the implementation's source file is included in the build. A return-type conflict with an included declaration is normally a compilation error, not this missing-definition example.
\end{commonerror}
...
If you are working on your own Windows computer and have not yet configured the CSC2059 development environment, follow the separate \textbf{Lab 0: CSC2059 Development Environment (Pre-lab setup)}.
...
\Needspace{48mm}
Once the relevant check passes, inspect your changes and commit your progress:
...
Commit once the relevant tests pass and earlier working behaviour still works.
...
Commit when the relevant tests pass and earlier working behaviour still works.
...
Remember to handle division by zero defensively. If the operator is not recognised, return \texttt{0.0}. Return that value to the caller; do not terminate the program with \texttt{exit}. The tests check an unsupported operator as well as the four supported ones.
...
Commit when the calculator tests pass and earlier working behaviour still works.
...
\begin{wrapupbox}
By now you have completed a small C++ library from a scaffold, learned how headers and source files work together, written your first defensive code, and pushed a submission through the same workflow you will use for the rest of this module.
