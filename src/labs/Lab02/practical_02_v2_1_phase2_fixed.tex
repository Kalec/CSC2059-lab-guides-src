...
\begin{tcolorbox}[
  ...
  boxrule=0.6pt,
  arc=2mm,
  left=2mm,right=2mm,top=4mm,bottom=3mm,
...
\begin{tikzpicture}[
  file/.style={draw=##1!75!black, fill=##1!7, rounded corners=2mm, very thick,
    minimum width=34mm, minimum height=15mm, align=center, font=\sffamily\small},
  action/.style={draw=CSCBlue!75!black, fill=CSCLightBlue, rounded corners=2mm, very thick,
    minimum width=32mm, minimum height=13mm, align=center, font=\sffamily\small\bfseries},
  output/.style={draw=CSCRed!75!black, fill=CSCRed!6, rounded corners=2mm, very thick,
    minimum width=34mm, minimum height=13mm, align=center, font=\sffamily\small},
  arrow/.style={-{Latex[length=3mm]}, very thick, draw=CSCBlue!70},
  include/.style={-{Latex[length=2.5mm]}, very thick, dashed, draw=black!55},
  note/.style={font=\sffamily\scriptsize, align=center, text=black!75}
]

% Source artefacts
\node[file=CSCBlue] (h) at (0,3.2) {\texttt{myFunctions.h}\\\footnotesize declarations};
\node[file=CSCGreen] (cpp) at (0,1.25) {\texttt{myFunctions.cpp}\\\footnotesize implementations};
\node[file=CSCAmber] (main) at (4.1,2.35) {\texttt{main.cpp}\\\footnotesize manual program};
\node[file=CSCPurple] (tests) at (4.1,0.15) {\texttt{tests/public\_tests.cpp}\\\footnotesize public tests};

% Build actions
\node[action] (makerun) at (8.0,2.35) {\texttt{make run}};
\node[action] (maketest) at (8.0,0.15) {\texttt{make test}};
\node[action] (buildrun) at (11.5,2.35) {compile\\+ link};
\node[action] (buildtest) at (11.5,0.15) {compile\\+ link};

% Outputs
\node[output] (program) at (15.0,2.35) {program\\executable};
\node[output] (testexe) at (15.0,0.15) {test\\executable};

% Header inclusion / relationship
\draw[include] (h) -- node[note, above]{declares} (cpp);
\draw[include] (h.east) -- ++(0.55,0) |- node[note, near end, above]{included} (main.west);
\draw[include] (h.east) -- ++(0.55,0) |- node[note, near end, below]{included} (tests.west);

% make run branch
\draw[arrow] (main) -- (makerun);
\draw[arrow] (cpp.east) -- ++(0.65,0) |- (makerun.west);
\draw[arrow] (makerun) -- (buildrun);
\draw[arrow] (buildrun) -- (program);

% make test branch
\draw[arrow] (tests) -- (maketest);
\draw[arrow] (cpp.east) -- ++(0.65,0) |- (maketest.west);
\draw[arrow] (maketest) -- (buildtest);
\draw[arrow] (buildtest) -- (testexe);

\node[note, below=6mm of maketest] {The same library code is used by both the manual program and the automated tests.};
\end{tikzpicture}%
}
\end{tcolorbox}
\caption{How the Practical 2 files work together when you run or test the project.}
