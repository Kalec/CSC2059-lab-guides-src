...
\centering
\begin{tikzpicture}[
  step/.style={circle, draw=##1, fill=##1!8, very thick, minimum size=22mm, align=center, font=\sffamily\bfseries\scriptsize, text=black},
  arrow/.style={-{Latex[length=3mm]}, thick, draw=CSCBlue!70},
  label/.style={font=\sffamily\small, align=center, text=black}
]
\node[step=CSCBlue] (read) {Read\\task};
\node[step=CSCGreen, right=14mm of read] (edit) {Edit\\code};
\node[step=CSCAmber, right=14mm of edit] (test) {Run\\\texttt{make test}};
\node[step=CSCRed, below=13mm of test] (fix) {Interpret\\errors};
\node[step=CSCPurple, left=14mm of fix] (commit) {Commit};
\node[step=CSCBlue, left=14mm of commit] (push) {Push};
\draw[arrow] (read) -- (edit);
\draw[arrow] (edit) -- (test);
\draw[arrow] (test) -- (fix);
\draw[arrow] (fix) -- (commit);
\draw[arrow] (commit) -- (push);
\draw[arrow] (push.west) .. controls +(-18mm,0) and +(-18mm,0) .. (read.west);
\node[label, below=5mm of commit] {Small steps. Frequent feedback. Regular Git history.};
\end{tikzpicture}
\caption{The CSC2059 edit--test--commit workflow used throughout the module.}
...
for tree={
  ...
  l sep=10mm,
  font=\ttfamily\small,
  edge path={\noexpand\path [draw, CSCBlue!55, thick]
    (!u.south west) +(4pt,0) |- (.child anchor)\forestoption{edge label};},
  if n children=0{tier=word}{},
}
[practical-02/
  [README.md, label=right:{\normalfont\scriptsize project overview and reminders}]
  [Makefile, label=right:{\normalfont\scriptsize build and test shortcuts}]
  [myFunctions.h, label=right:{\normalfont\scriptsize function declarations}]
  [myFunctions.cpp, label=right:{\normalfont\scriptsize function implementations}]
  [main.cpp, label=right:{\normalfont\scriptsize program entry point}]
  [tests/
    [public\_tests.cpp, label=right:{\normalfont\scriptsize automated public tests}]
  ]
  [.github/, label=right:{\normalfont\scriptsize GitHub/Classroom50 configuration}]
]
\end{forest}
...
\centering
\begin{tikzpicture}[
  file/.style={draw=##1, fill=##1!7, rounded corners=2mm, very thick, minimum width=31mm, minimum height=17mm, align=center, font=\sffamily\small, text=black},
  process/.style={draw=CSCBlue, fill=CSCLightBlue, rounded corners=2mm, very thick, minimum width=34mm, minimum height=14mm, align=center, font=\sffamily\small\bfseries},
  arrow/.style={-{Latex[length=3mm]}, thick, draw=CSCBlue!75},
  note/.style={font=\sffamily\scriptsize, align=center, text=black!80}
]
\node[file=CSCBlue] (header) {\texttt{myFunctions.h}\\\footnotesize declarations};
\node[file=CSCGreen, right=11mm of header] (source) {\texttt{myFunctions.cpp}\\\footnotesize implementations};
\node[file=CSCAmber, right=11mm of source] (main) {\texttt{main.cpp}\\\footnotesize program code};
\node[file=CSCPurple, below=16mm of source] (tests) {\texttt{tests/public\_tests.cpp}\\\footnotesize test code};
\node[process, right=13mm of main] (build) {compiler + linker};
\node[process, below=14mm of build] (result) {program / test executable};

\draw[arrow] (header) -- node[note, above]{declares} (source);
\draw[arrow] (source) -- node[note, above]{compiled with} (main);
\draw[arrow] (main) -- (build);
\draw[arrow] (tests.east) -| node[note, near start, below]{\texttt{make test}} (build.south);
\draw[arrow] (build) -- (result);
\draw[arrow, dashed] (header.south) |- node[note, near start, left]{included by} (tests.west);
\draw[arrow, dashed] (header.south) |- (main.south);
\end{tikzpicture}
\caption{How the header file, implementation file, main program, and public tests fit together.}
\label{fig:compilation-model}
\end{figure}%
}
...
Open the Classroom50 link provided by your lecturer:

\begin{terminal}
<CLASSROOM50_LINK>
\end{terminal}

If prompted, log in using your GitHub account. Classroom50 will automatically create a private repository for you containing the files required for this practical.
...
\subsection{1. Read the instructions}
...
\subsection{2. Implement the function}
...
\subsection{4. Commit your progress}
...
As a final check, visit your repository in Classroom50 and verify that your latest commit is visible online.
...
This usually indicates that a compiler toolchain has not been installed.

Refer to Appendix A for instructions on setting up a development environment on your own computer.
\end{commonerror}
...
\section{Setting Up Your Own Development Environment}

This appendix is intended for students who wish to complete practical work on their own computers.

Students working in University laboratories should already have the required software installed.

\subsection{Windows Installation}

For this module we recommend using the GCC compiler distributed through MSYS2.
...
You may be asked to restart the terminal and repeat the command.

Install the required packages:

\begin{bashcode}
pacman -S --needed base-devel mingw-w64-ucrt-x86_64-gcc mingw-w64-ucrt-x86_64-gdb mingw-w64-ucrt-x86_64-make mingw-w64-ucrt-x86_64-cmake
\end{bashcode}

Add the compiler binaries to your system PATH:

\begin{terminal}
C:\Apps\MSYS64\ucrt64\bin
\end{terminal}
...
This installs Apple's Command Line Tools, including the compiler and build tools required for this module.
...
\begin{tipbox}
On macOS, the \texttt{g++} command may point to Apple's \texttt{clang++} compiler. This is acceptable for CSC2059.
\end{tipbox}
...
\begin{itemize}
  \item \textbf{C/C++} by Microsoft: provides syntax highlighting, code completion, and debugging support;
  \item \textbf{GitHub Pull Requests and Issues}: useful for interacting with GitHub repositories directly from VS Code.
\end{itemize}
...
If all commands display version information, your environment is correctly configured.

\subsection{Testing Your Installation}

Open your Practical 2 repository and run:
...
\begin{terminal}
CSC2059 Practical 2: C++ Functions
\end{terminal}
...
Initially, some tests are expected to fail because the practical scaffold contains incomplete implementations.

As you complete the exercises, progressively more tests should pass. Eventually, you should see:

\begin{terminal}
All public tests passed.
\end{terminal}

Congratulations. Your development environment is ready to support the remainder of the module.
