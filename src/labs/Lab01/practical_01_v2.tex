...
\node[box, right=26mm of local] (remote) {Remote repository\\\footnotesize on GitHub};
...
\end{itemize}
Don't worry if you've never used Git before. By the end of this lab, you will have created repositories, made commits, worked with branches, resolved a merge conflict, and prepared your GitHub account for Practical 2, where you will clone your individual repository and push work online.
...
\begin{enumerate}
  ...
  \item Use Git through the command line, with an optional Visual Studio Code GUI activity.
  \item Create, merge, and manage branches.
  \item Resolve a simple merge conflict.
  \item Prepare your GitHub account for cloning and pushing in Practical 2.
...
This is a preview of the module workflow. Today we practise local Git; testing C++ and pushing to GitHub begin in Practical 2:
...
\begin{itemize}
  ...
  \item Use Git from the command line and optionally the Visual Studio Code GUI.
...
If you are working on your own Windows laptop or PC, follow the separate \textbf{Lab 0: CSC2059 Development Environment (Pre-lab setup)} before continuing with this practical.
...
The required software is already installed on the School laboratory computers. However, Visual Studio Code may initially open PowerShell.
...
Complete the macOS or Linux setup in Lab 0 before this session. Use the normal integrated terminal in Visual Studio Code; no special CSC2059 terminal profile is required.
...
All users should also check that Git and the VS Code launcher are available:

\begin{terminalplain}
git \mintinline{text}{--version}

code \mintinline{text}{--version}
\end{terminalplain}

Both commands should print version information. If either is missing, return to Lab 0 or ask a demonstrator before continuing; the editor configuration below requires the code command.
...
\begin{itemize}
  \item Configure your commit author name.
...
Global settings provide \textbf{your own defaults across repositories}. A repository can override them locally. Your commit author name is a readable name, not your GitHub username. Use the same university email address that you add and verify in GitHub later in this practical.

\begin{importantnote}
\textbf{Your settings on School computers}

Both the terminal and VS Code Git interface use your personal \mintinline{text}{I:\.gitconfig} file. These Git settings persist across laboratory resets unless you delete that file from the root of your I: drive. Keep it. On your own computer, complete the separate setup in Lab 0; it does not use the laboratory I: drive.
\end{importantnote}
...
\subsubsection{Step 3: Choose the Commit Editor}
Run:
\begin{bashcode}
git config --global core.editor "code --wait"
\end{bashcode}
Git will open VS Code when it needs a message. Save and close that editor tab when finished; Git waits for the tab to close.

\subsubsection{Step 4: Windows Line Endings}
\textbf{Windows users only} should run:
\begin{bashcode}
git config --global core.autocrlf true
\end{bashcode}
Windows and Unix systems conventionally use different line endings (CRLF and LF). This setting gives the two Windows Git installations the same conversion policy. macOS and Linux users should skip this step.

\subsubsection{Step 5: Verify Your Configuration}
Run:
...
Your university email address and \mintinline{text}{core.editor=code --wait} appear in the output. On Windows, also check \mintinline{text}{core.autocrlf=true}.

If these checks pass, your global Git configuration is working correctly.

\subsection{Repository-Specific Configuration}
Sometimes you may want different defaults for a single repository. The following is an explanation, not a step to perform now: these commands only work inside an existing repository.
...
A separate personal project might need a different email address. Local settings can override your global defaults for that project. They are not needed to survive laboratory resets: your global settings remain on I: unless you delete the configuration file.
\end{importantnote}
...
\end{terminalplain}
Set repository-specific commit author name:
...
\begin{terminalplain}
  git config \mintinline{text}{--global} user.email "your.email@example.com"
\end{terminalplain}
You may need to configure Git again on a different personal computer, or if you delete your configuration file. Laboratory resets do not delete the settings stored on your I: drive.
...
These are the defaults for new commits. We will check the recorded name and email address after your first commit.
...
No commits yet
\end{terminalplain}
The option \texttt{-b main} chose the initial branch name. There is no commit history yet.
...
\subsection{Step 9: Ignore a Disposable File}
In VS Code, create and save a file called \texttt{scratch.tmp}. Run \texttt{git status}: it appears as untracked, meaning it has not been added to Git.

Now create a file named exactly \texttt{.gitignore} in the same folder, containing:
\begin{terminalplain}
scratch.tmp
\end{terminalplain}
Save it and run \texttt{git status} again. The scratch file is omitted, but \texttt{.gitignore} and \texttt{hello.txt} are still untracked.

An ignore rule excludes matching untracked files from normal status and staging. The scratch file still exists on disk. Commit \texttt{.gitignore} so its rules travel with the repository. An ignore rule does not stop Git tracking an already committed file.

\subsection{Step 10: Stage the Files}
Run:

\begin{terminalplain}
git add hello.txt .gitignore
\end{terminalplain}
The command will normally produce no output.

This is perfectly normal.

\subsection{Step 11: Check Git Again}
Before running the next command, make a prediction.
...
\hspace{1cm}\textcolor{GitGreen}{new file: .gitignore}

\hspace{1cm}\textcolor{GitGreen}{new file: hello.txt}
\end{terminalplain}
Excellent.

Git is now preparing to include both files in the next commit.

\subsection{Step 12: Create Your First Commit}
...
\subsection{Step 13: Check Git One More Time}
Run:
...
\begin{itemize}
  \item No tracked changes are staged or unstaged.
  \item No unignored untracked files are reported.
  \item The ignored scratch file still exists.
\end{itemize}
Git sees saved files on disk, not unsaved editor changes. A clean status does not mean a commit has been uploaded. Your first snapshot is local.
\subsection{Step 14: View Your Commit History}
Run:

\begin{terminalplain}
git log
\end{terminalplain}
Git will display detailed information about your commit. Check that the Author line contains your own name and university email address. If not, ask a demonstrator before making further commits.
...
Enter a message in the VS Code tab, save it and close that tab. Git waits for it to close. To cancel an ordinary commit, remove all non-comment message text, save and close the tab; Git aborts an empty-message commit. A merge may have a pre-filled message: simply closing it does not cancel the merge commit.
\end{commonerror}
\begin{commonerror}
...
It means no changes are ready for that commit. Save your editor files and inspect the status; ignored files are not included.
...
This is where Git starts becoming genuinely useful.
...
\end{terminalplain}
The changes are now staged and ready to be included in the next commit. Inspect them with:
\begin{bashcode}
git diff --staged
\end{bashcode}
\texttt{git diff} shows unstaged changes to tracked files; \mintinline{text}{git diff --staged} shows what the next commit will contain. The first command may be empty after staging even though there are changes ready to commit.
...
This means Git reports no pending tracked changes or unignored untracked files. Save editor changes before checking; the ignored scratch file remains on disk.
...
Git only sees the saved version. If you have already staged the changes, use \mintinline{text}{git diff --staged} instead.
\end{commonerror}
\begin{commonerror}
...
\begin{terminalplain}
  nothing to commit, working tree clean
\end{terminalplain}
then Git sees no pending tracked changes or unignored untracked files. Check that you saved the intended edits; clean status alone does not prove they were committed.
\end{commonerror}
...
\end{learningoutcomes}
\subsection*{Before Branching: Both Routes Meet Here}
If you skipped the optional GUI activity, add \texttt{Version control saves future headaches.} as the third line of \texttt{hello.txt}, save it, and run:
\begin{bashcode}
git add hello.txt
git commit -m "Add version control note"
\end{bashcode}
If you already made that GUI commit, do not repeat it. Both routes should now have three commits: \texttt{Add hello message}, \texttt{Add Git description}, and \texttt{Add version control note}. Check with \mintinline{text}{git log --oneline} and confirm \texttt{git status} is clean.
...
These markers were added by Git to identify the conflicting versions. Remove them to produce the intended resolved file before staging.
\end{importantnote}
...
\end{terminalplain}
which attempts to complete the current merge after conflicts have been resolved and staged. It reports an error if unresolved paths remain and may open the commit-message editor.
...
Git trusts you when you stage a resolved file: it can commit leftover marker text. Remove every marker, save and inspect the file before staging it.
\end{commonerror}
...
\subsection{Final Step: Register Your GitHub Account}

Before completing this section, you \textbf{must register your GitHub username with the CSC2059 teaching team}.

We use this information to associate your GitHub account with your student record and provide access to CSC2059 practical and assessment repositories through Classroom50.

Complete the \textbf{CSC2059 GitHub Account Registration} form using the link or QR code below:

\begin{center}
\url{https://forms.cloud.microsoft/e/i6RBy78Pmj}
\end{center}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.28\textwidth]{../../../assets/figures/practical01/QRCode for CSC2059 GitHub Account Registration.png}
\caption{CSC2059 GitHub Account Registration form}
\label{fig:GitHubAccountRegistration}
\end{figure}

\begin{importantnote}
\textbf{Enter Your GitHub Username}\par
\medskip

Enter your \textbf{GitHub username}, not your display name or email address.

Your username is the part that appears in your GitHub profile address. For example, if your profile is:
...
\begin{reflectionbox}
\textbf{Required Checkpoint}\par
\medskip

Before continuing, confirm that you have submitted the CSC2059 GitHub Account Registration form using your correct GitHub username.
\end{reflectionbox}
...
\begin{itemize}
  ...
  \item \faCheck\ Registered your GitHub username with the CSC2059 teaching team.
...
\begin{terminalplain}
git diff \mintinline{text}{--staged}
\end{terminalplain}
These commands show the repository status and the staged changes proposed for the next commit.
