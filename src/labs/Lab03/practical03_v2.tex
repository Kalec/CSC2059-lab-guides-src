...
Use the Canvas link to accept the Practical 3 assignment in Classroom50. Once your repository has been created, clone it to a suitable location on your machine.

\begin{bashcode}
git clone <your-practical-3-repository-url>
cd <your-practical-3-repository-name>
code .
\end{bashcode}
...
Before editing any code, take a moment to look around the repository. The layout is deliberately similar to the practicals you have already seen.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{../../../assets/figures/practical03/var_in_memory.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{../../../assets/figures/practical03/indirect_assignment.png}
...
The file contains two lines of commented-out code. These lines are unsafe on purpose. To see why, uncomment one unsafe line at a time, run \texttt{make demo04}, observe what happens, and then comment the line out again before moving on.

\begin{importantnote}
Unsafe pointer behaviour may crash the program, print unexpected output, or appear to work. If the program crashes, that is part of the lesson. Restore the comments before continuing with the rest of the practical.
\end{importantnote}
...
\begin{workflowreminder}
For each implementation task, begin by running the relevant test target. Read the failing test names, make a small change, and run the tests again.

A failing test is not a disaster. It is information.
\end{workflowreminder}
